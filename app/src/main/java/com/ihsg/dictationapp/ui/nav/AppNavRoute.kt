...
data object ValidationPageRoute : AppNavRoute(
    "validation?$ARG_BOOK_ID={$ARG_BOOK_ID}&$ARG_GRADE_ID={$ARG_GRADE_ID}&$ARG_LESSON_ID={$ARG_LESSON_ID}"
){
    fun getArguments() = listOf(
        navArgument(ARG_BOOK_ID) {
            type = NavType.LongType
    ...
    fun getLessonId(arguments: Bundle?) = arguments?.getLong(ARG_LESSON_ID) ?: 0L
    fun getPathWithArgs(bookId: Long, gradeId: Long, lessonId: Long): String =
        "validation?$ARG_BOOK_ID=$bookId&$ARG_GRADE_ID=$gradeId&$ARG_LESSON_ID=$lessonId"
}
